...
def generate_report(selected_city, predicted_lst, wbgt, 
                    risk_level, shap_values, feature_cols,
                    feature_info, input_values, metadata,
    ...
    abs_shap = [abs(v) for i,v in enumerate(shap_values) 
                if feature_cols[i] != 'city_encoded']
    feat_names = [f for f in feature_cols if f != 'city_encoded']
    top_driver = feat_names[abs_shap.index(max(abs_shap))]
    top_shap = max(abs_shap)
...
City Analyzed    : {selected_city}
Model            : XGBoost (R²={metadata['r2']:.3f}, RMSE={metadata['rmse']:.2f}°C)
Data Source      : Landsat 8/9 via Google Earth Engine + ERA5-Land

--------------------------------------------------------------------------------
1. PREDICTION SUMMARY
--------------------------------------------------------------------------------

Predicted Land Surface Temperature (LST) : {predicted_lst:.1f}°C
City Mean LST                             : {metadata.get('city_feature_stats',{}).get(selected_city,{}).get('LST',{}).get('mean', 40.5):.1f}°C
Deviation from Mean                       : {predicted_lst - metadata.get('city_feature_stats',{}).get(selected_city,{}).get('LST',{}).get('mean', 40.5):+.1f}°C
Heat Stress Index                         : {wbgt:.1f}°C
Heat Risk Classification                  : {risk_level}

--------------------------------------------------------------------------------
2. ENVIRONMENTAL CONDITIONS ANALYZED
--------------------------------------------------------------------------------
"""
    for feat in feat_names:
        val = input_values.get(feat, 0)
        info = feature_info.get(feat, {})
        name = info.get('full', feat)
        unit = info.get('unit', '')
        report += f"  {name:<45} : {val:.4f} {unit}\n"
    
    report += f"""
--------------------------------------------------------------------------------
3. KEY HEAT DRIVERS (SHAP Analysis)
--------------------------------------------------------------------------------

Primary Driver : {top_driver} (SHAP contribution: +{top_shap:.2f}°C)

Feature Contributions (ranked by importance):
"""
    sorted_feats = sorted(zip(feat_names, abs_shap), 
                         key=lambda x: x[1], reverse=True)
    for fname, fval in sorted_feats:
        bar = '█' * int(fval * 10)
        report += f"  {fname:<20} {bar:<15} {fval:+.3f}°C\n"
    
    report += f"""
--------------------------------------------------------------------------------
4. COOLING INTERVENTION RECOMMENDATIONS
--------------------------------------------------------------------------------

Based on the analysis of {selected_city}, the following interventions 
are recommended in order of projected impact:

  1. PLANT URBAN FOREST
     Action  : Increase vegetation cover (NDVI +0.15, NDBI -0.05)
     Impact  : Estimated LST reduction via evaporative cooling
     Target  : Parks, road medians, rooftop gardens

  2. INSTALL COOL ROOFS  
     Action  : High-albedo roofing materials (NDWI +0.05, NDBI -0.10)
     Impact  : Reduces solar heat absorption in built-up zones
     Target  : Commercial and industrial rooftops

  3. RESTORE WATER BODIES
     Action  : Restore lakes and water features (NDWI +0.12)
     Impact  : Latent heat cooling effect on surrounding areas
     Target  : Degraded urban lakes and stormwater channels

--------------------------------------------------------------------------------
5. HEAT STRESS RISK ASSESSMENT
--------------------------------------------------------------------------------

Heat Stress Index : {wbgt:.1f}°C ({risk_level})

Risk Thresholds (General outdoor heat stress guidelines):
  < 28°C  — Low Risk      : Normal outdoor activity
  28-32°C — Moderate Risk : Caution advised for outdoor workers  
  32-35°C — High Risk     : Limit outdoor exertion
  > 35°C  — Extreme Risk  : Outdoor work dangerous

Current Status : {"⚠️  IMMEDIATE ACTION RECOMMENDED" if wbgt > 35 else "Monitor conditions closely"}

--------------------------------------------------------------------------------
6. MODEL INFORMATION
--------------------------------------------------------------------------------

Algorithm          : XGBoost Regressor + Physics-Informed Neural Network
Training Cities    : Hyderabad, Delhi, Mumbai, Chennai
Training Samples   : {metadata.get('n_samples', 20164):,}
Validation Method  : Per-city spatial block GroupShuffleSplit
XGBoost R²         : {metadata['r2']:.3f}
XGBoost RMSE       : {metadata['rmse']:.2f}°C
Features Used      : NDVI, NDWI, NDBI, Albedo, MNDWI
Data Source        : Landsat 8/9 C02 T1 L2 + ECMWF ERA5-Land

--------------------------------------------------------------------------------
7. DISCLAIMER
--------------------------------------------------------------------------------

This report is generated by an AI/ML system trained on satellite data.
Predictions are based on spectral indices derived from Landsat composites
(Apr-Jun 2025). Absolute LST values may vary ±{metadata['rmse']:.1f}°C from 
ground truth. This tool is intended for urban planning guidance and 
research purposes. Field validation is recommended before policy action.

================================================================================
        Generated by UHI Predictor — ISRO BAH 2026 Submission
...
def render_simulator_tab(ctx, model, scaler, metadata, explainer, forest_satellite_b64, cool_roofs_satellite_b64, water_satellite_b64):
    ...
    heat_stress_val = ctx['heat_stress']
    risk_level = ctx['heat_stress_label']

    report_text = generate_report(
        selected_city=selected_city,
        predicted_lst=predicted_lst,
        wbgt=heat_stress_val,
    ...
    st.download_button(
        ...
        data=report_text,
        file_name=f"UHI_Report_{selected_city}_{datetime.now().strftime('%Y%m%d_%H%M')}.txt",
        mime="text/plain",
        use_container_width=True,
        type="primary",
    )
    st.caption(
        "Report includes LST prediction, SHAP feature attribution, "
        "heat stress assessment, and cooling intervention recommendations."
    )
